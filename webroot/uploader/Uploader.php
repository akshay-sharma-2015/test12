...
class Uploader {
	...
	{	
		//Connect to database
		$this->db = @mysqli_connect(DB_HOST , DB_USER , DB_PASS) or die("<h1>Error establishing a database connection</h1>");
		@mysqli_select_db($this->db, DB_NAME) or die("<h1>Error establishing a database connection</h1>");
	...
	public function save_info($data)
	{
		...
		$title = mysqli_real_escape_string($this->db,$this->xss_clean($title));
		$description = mysqli_real_escape_string($this->db,$this->xss_clean($description));
	...
	public function sort($order)
	{
		...
		foreach ($order['sort'] as $key => $id) {
			@mysqli_query($this->db,"UPDATE $this->table SET display_order = $key WHERE id = $id LIMIT 1");
		}
	}
	...
	public function upload_limit($object_id) {
		$rows = 0;
		//MYSQL
		$result = @mysqli_query($this->db,"SELECT id FROM $this->table WHERE object_id = $object_id");
	...
	public function load($object_id, $user_id = NULL, $full = FALSE, $type = 'all', $type_id = 1)
	{
		...
		$result = @mysqli_query($this->db,"SELECT * FROM $this->table WHERE object_id = $object_id ORDER BY display_order ASC");
		if (@mysqli_num_rows($result)) {
	...
	public function process_uploaded_files($files, $object_id, $user_id = 0, $type_id = 1)
	{
		...
		foreach ($files as $key => $file) {
			...
			foreach ($fields as $key => $field) 
				$values[] = '"' . mysqli_real_escape_string($this->db,$field) .'"';
		   ...
		   if (@mysqli_query($this->db,$query)) {
		    	$result[] = array(
					'id'           =>  mysqli_insert_id($this->db), // Insert ID !!
					'name'         => $file->name,
	...
	public function delete($file, $image = false)
	{
		...
		//MYSQL
		return @mysqli_query($this->db,"DELETE FROM $this->table WHERE file = '$file' LIMIT 1");
	}
	...
	public function delete_all($object_id)
	{
		...
		//MYSQL
		return @mysqli_query($this->db,"DELETE FROM $this->table WHERE object_id = '$object_id'");
	}
...
function show_files($object_id, $user_id = null, $type_id = 1, $files_type = 'all') {
	...
	if ($files_type == 'images' or $files_type == 'all') 
	{
		...
		if (!empty($files) and is_array($files)) {
			foreach ($files as $key => $file) 
				echo '<a href="'.WEBSITE_URL.'files/'.$file['file'].'" data-lightbox="gallery-'.$file['object_id'].'" title="'.$file['title'].' - '.$file['description'].'"><img src="'.WEBSITE_URL.'files/thumbnail/'.$file['file'].'" width="80" height="80"></a>';
