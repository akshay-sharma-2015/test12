    ...
    function create($field='captcha', $config=array()) {
        ...
        switch( $this->_config['type']):
            case 'image':
                ...
                $html .= $this->Html->image(array('controller'=>$controller, 'action'=>$action, '?'=> $qstring));
                $html .= $this->Html->link( $this->_config['reload_txt'], '#', array('class' => 'creload', 'escape' => false));
                $html .= $this->Form->input($field, array('autocomplete'=>'off','label'=> $this->_config['clabel'],'class'=>'form-control','data-ng-model' => "formData.securitycode",'required' => true));
            break;
            case 'math':
